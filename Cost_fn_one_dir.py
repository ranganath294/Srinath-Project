...
def cost_oned (passengerarrival,distance,frequency,timeperiod,link_traveltime,alightrate,fare, files,direcn,purpose):
    ...
    time_period['Headway_in_hours'] = (1 / (frequency)).astype(float).round(2)
    ...
    if purpose=='GA':
        ...
        # files export
        path = r'D:\Function files\Input_files_holding'
        ...
        if direcn == 'DN':
            ...
            data = [
                ['Parameter', 'Value'],
                ['Total waiting time cost', '₹' + str(float((np.ceil(Totcost_waiting))))],
                ['Total cost of in vehicle time', '₹' + str(float((np.ceil(Totcost_inveh))))],
                ['Total passenger lost', Totpasslost],
                ['User Cost', '₹' + str(float((cuser.round(0))))],
                ['Operator Penalty Cost for passenger lost', '₹' + str(float((Totpasslost * penalty)))],
                ['Vehicle Kilometre-run', str(float((totalkilometrerun))) + ' Km'],
                ['Cost of fuel', '₹' + str(float((np.ceil(fuelcostday))))],
                ['Cost of vehicle maintenance', '₹' + str(float((maintenancecost * total_trips)))],
                ['Vehicle depreciation cost', '₹' + str(float((vehdepreciation * total_trips)))],
                ['Crew cost', '₹' + str(float((crewcost * total_trips)))],
                ['Operator Cost for bus operation', '₹' + str(float((np.ceil(coperator))))],
                ['Total cost in down direction', '₹' + str(float((t_cost)))]
            ]

            directory = r'D:\Function files\Output GA'
            filename = 'cost_calculations_down.csv'
            ...
            print(f'Cost calculations DOWN Direction saved to {filepath}')


            if purpose == 'optmised frequency':
                link_travel_tr.to_csv(r'D:\Function files\Input_files_holding\link_travel_trDN.csv', index=False)
                arrivalrate_tr.to_csv(r'D:\Function files\Input_files_holding\arrivalrate_trDN.csv', index=False)
                alightrate_tr.to_csv(r'D:\Function files\Input_files_holding\alightrate_trDN.csv', index=False)
                travel_time_tot.to_csv(r'D:\Function files\Input_files_holding\travel_time_totDN.csv', index=False)
                tot_cost_hr.to_csv(r'D:\Function files\Input_files_holding\tot_cost_hr_DN.csv', index=False)

                headway1.to_csv(r'D:\Function files\Input_files_holding\HeadwayDN.csv', index=False)
                departuretime.to_csv(r'D:\Function files\Input_files_holding\departuretimeDN.csv', index=False)

                p_arrival.to_csv(r'D:\Function files\Input_files_holding\p_arrivalDN.csv', index=False)
                p_waiting.to_csv(r'D:\Function files\Input_files_holding\p_waitingDN.csv', index=False)
                p_alight.to_csv(r'D:\Function files\Input_files_holding\p_alightDN.csv', index=False)
                p_board.to_csv(r'D:\Function files\Input_files_holding\p_boardDN.csv', index=False)
                link_occp.to_csv(r'D:\Function files\Input_files_holding\link_occpDN.csv', index=False)
                waitingtime_tr.to_csv(r'D:\Function files\Input_files_holding\waitingtimeDN.csv', index=False)
                cost_waitingtime.to_csv(r'D:\Function files\Input_files_holding\cost_waitingtimeDN.csv', index=False)
                p_cantboard.to_csv(r'D:\Function files\Input_files_holding\p_cantboardDN.csv', index=False)
                p_lost.to_csv(r'D:\Function files\Input_files_holding\ p_lostDN.csv', index=False)
                d_time.to_csv(r'D:\Function files\Input_files_holding\d_timeDN.csv', index=False)
                stoparrival.to_csv(r'D:\Function files\Input_files_holding\stoparrivalDN.csv', index=False)
                headway.to_csv(r'D:\Function files\Input_files_holding\headway_StopDN.csv', index=False)
                traveltime.to_csv(r'D:\Function files\Input_files_holding\traveltimeDN.csv', index=False)
                load_fact.to_csv(r'D:\Function files\Input_files_holding\load_factDN.csv', index=False)
                invehtime.to_csv(r'D:\Function files\Input_files_holding\invehtimeDN.csv', index=False)
                cost_inveh.to_csv(r'D:\Function files\Input_files_holding\cost_invehDN.csv', index=False)
                p_sit.to_csv(r'D:\Function files\Input_files_holding\p_sitDN.csv', index=False)
                p_stand.to_csv(r'D:\Function files\Input_files_holding\ p_standDN.csv', index=False)
                revenue.to_csv(r'D:\Function files\Input_files_holding\revenueDN.csv', index=False)
                p_cantboard_1.to_csv(r'D:\Function files\Input_files_holding\ p_cantboard_1DN.csv', index=False)
                p_cantboard_2.to_csv(r'D:\Function files\Input_files_holding\p_cantboard_2DN.csv', index=False)
                p_cantboard_0.to_csv(r'D:\Function files\Input_files_holding\p_cantboard_0DN.csv', index=False)
                despatch.to_csv(r'D:\Function files\Input_files_holding\despatchDN.csv', index=False)
            ...
            data = [
                ...
                ['Crew cost', '₹' + str(float((crewcost * total_trips)))],
                ['Operator Cost for bus operation', '₹' + str(float((np.ceil(coperator))))],
                ['Total cost in up direction', '₹' + str(float((t_cost)))]
            ]

            directory = r'D:\Function files\Output GA'
            filename = 'cost_calculations_up.csv'
            filepath = os.path.join(directory, filename)
            ...
            print(f'Cost calculations UP Direction saved to {filepath}')


            if purpose == 'optmised frequency':
                link_travel_tr.to_csv(r'D:\Function files\Input_files_holding\link_travel_trUP.csv', index=False)
                arrivalrate_tr.to_csv(r'D:\Function files\Input_files_holding\arrivalrate_trUP.csv', index=False)
                alightrate_tr.to_csv(r'D:\Function files\Input_files_holding\alightrate_trUP.csv', index=False)
                travel_time_tot.to_csv(r'D:\Function files\Input_files_holding\travel_time_totUP.csv', index=False)
                tot_cost_hr.to_csv(r'D:\Function files\Input_files_holding\tot_cost_hr_UP.csv', index=False)

                headway1.to_csv(r'D:\Function files\Input_files_holding\HeadwayUP.csv', index=False)
                departuretime.to_csv(r'D:\Function files\Input_files_holding\departuretimeUP.csv', index=False)

                p_arrival.to_csv(r'D:\Function files\Input_files_holding\p_arrivalUP.csv', index=False)
                p_waiting.to_csv(r'D:\Function files\Input_files_holding\p_waitingUP.csv', index=False)
                p_alight.to_csv(r'D:\Function files\Input_files_holding\p_alightUP.csv', index=False)
                p_board.to_csv(r'D:\Function files\Input_files_holding\p_boardUP.csv', index=False)
                link_occp.to_csv(r'D:\Function files\Input_files_holding\link_occpUP.csv', index=False)
                waitingtime_tr.to_csv(r'D:\Function files\Input_files_holding\waitingtimeUP.csv', index=False)
                cost_waitingtime.to_csv(r'D:\Function files\Input_files_holding\cost_waitingtimeUP.csv', index=False)
                p_cantboard.to_csv(r'D:\Function files\Input_files_holding\p_cantboardUP.csv', index=False)
                p_lost.to_csv(r'D:\Function files\Input_files_holding\ p_lostUP.csv', index=False)
                d_time.to_csv(r'D:\Function files\Input_files_holding\d_timeUP.csv', index=False)
                stoparrival.to_csv(r'D:\Function files\Input_files_holding\stoparrivalUP.csv', index=False)
                headway.to_csv(r'D:\Function files\Input_files_holding\headway_StopUP.csv', index=False)
                traveltime.to_csv(r'D:\Function files\Input_files_holding\traveltimeUP.csv', index=False)
                load_fact.to_csv(r'D:\Function files\Input_files_holding\load_factUP.csv', index=False)
                invehtime.to_csv(r'D:\Function files\Input_files_holding\invehtimeUP.csv', index=False)
                cost_inveh.to_csv(r'D:\Function files\Input_files_holding\cost_invehUP.csv', index=False)
                p_sit.to_csv(r'D:\Function files\Input_files_holding\p_sitUP.csv', index=False)
                p_stand.to_csv(r'D:\Function files\Input_files_holding\ p_standUP.csv', index=False)
                revenue.to_csv(r'D:\Function files\Input_files_holding\revenueUP.csv', index=False)
                p_cantboard_1.to_csv(r'D:\Function files\Input_files_holding\ p_cantboard_1UP.csv', index=False)
                p_cantboard_2.to_csv(r'D:\Function files\Input_files_holding\p_cantboard_2UP.csv', index=False)
                p_cantboard_0.to_csv(r'D:\Function files\Input_files_holding\p_cantboard_0UP.csv', index=False)
                despatch.to_csv(r'D:\Function files\Input_files_holding\despatchUP.csv', index=False)
            else:
                pass
